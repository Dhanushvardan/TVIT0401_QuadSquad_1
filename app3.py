...
# simage = Image.open(r'y.jpg')
s1 = cv2.imread(r'y.jpg')
s2 = cv2.imread(r'y2.jpg')
s3 = cv2.imread(r'y3.jpg')
...
class OpenCamera ():
    ...
    def recv(self):
        ...
        st.write("s3")
        st.image(s3)
        option = st.selectbox('select an image',("s1","s2","s3"))
        if option:
            if option == "s1":
                    img = cv2.imread(r'y.jpg')
            if option == "s2":
                    img = cv2.imread(r'y2.jpg')
            if option == "s3":
                img = cv2.imread(r'y3.jpg')
            while True:
                # img = cv2.imread(img)
                # if not success:
